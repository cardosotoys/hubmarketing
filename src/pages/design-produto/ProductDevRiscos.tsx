...
export default function ProductDevRiscos() {
  const { profile } = useAuth();
  const { item } = useProductDevWorkspace();
  const [risks, setRisks] = useState<ProductDevRisk[]>([]);
  const [profiles, setProfiles] = useState<Profile[]>([]);
  const [editing, setEditing] = useState<ProductDevRisk | 'new' | null>(null);

  async function load() {
    const [risksRes, profilesRes] = await Promise.all([
      supabase.from('product_dev_risks').select('*').eq('item_id', item.id).order('created_at'),
      supabase.from('profiles').select('*'),
    ]);
    setRisks((risksRes.data as ProductDevRisk[]) ?? []);
...
function RiskFormModal({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (isEdit && risk) {
      await supabase.from('product_dev_risks').update({ ...fields, updated_at: new Date().toISOString() }).eq('id', risk.id);
    } else {
      await supabase.from('product_dev_risks').insert({ ...fields, created_by: actorId });
    }
    await logActivity({ actorId, actionText: isEdit ? 'Risco atualizado' : 'Risco registrado', detail: description, productDevItemId: itemId });
    onSaved();
  }

  async function handleDelete() {
    if (!risk) return;
    await supabase.from('product_dev_risks').delete().eq('id', risk.id);
    await logActivity({ actorId, actionText: 'Risco removido', detail: risk.description, productDevItemId: itemId });
    onSaved();
  }
