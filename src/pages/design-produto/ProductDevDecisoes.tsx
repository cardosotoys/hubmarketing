...
import type { ProductDevDecision } from '../../types/database';

export default function ProductDevDecisoes() {
  const { profile } = useAuth();
  const { item } = useProductDevWorkspace();
  const [decisions, setDecisions] = useState<(ProductDevDecision & { created_by_profile: { name: string } | null })[]>([]);
  const [showNew, setShowNew] = useState(false);

  async function load() {
    const { data } = await supabase
      .from('product_dev_decisions')
      .select('*, created_by_profile:profiles(name)')
      .eq('item_id', item.id)
      .order('decided_at', { ascending: false });
    setDecisions((data as (ProductDevDecision & { created_by_profile: { name: string } | null })[]) ?? []);
  }
...
function NewDecisionModal({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    await supabase.from('product_dev_decisions').insert({
      item_id: itemId,
      context,
      alternatives,
      choice: choice.trim(),
    ...
    await logActivity({ actorId, actionText: 'Decisão registrada', detail: choice, productDevItemId: itemId });
    onSaved();
  }
